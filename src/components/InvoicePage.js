...
import customerReducer, { fetchCustomers } from '../features/customer/customerReducer';
...
function InvoicePage() {
    ...
    const handleOnSellerChange = (e) => {
        let seller = sellerList.find(
            (seller) => seller.name === e.target.value
        );

        setCustomer({ ...customer, sellerName: e.target.value, sellerAddress: seller.address, sellerMobile: seller.mobile, sellerEmail: seller.email, sellerPanNo: seller.panNo, sellerGstNo: seller.gstNo });   
    }
    ...
    const handleOnChange = (e) => {
        ...
        if (e.target.name === "customerName" && e.target.value !== "") {
            ...
            setCustomer({ ...customer,customerName:selectedCustomer.customerName, customerAddress: selectedCustomer.customerAddress, customerMobile: selectedCustomer.customerMobile, customerEmail: selectedCustomer.customerEmail });
        }
    }
    ...
    const handleItemChange = (e, index) => {
        ...
        if(e.target.name === 'name'){
            ...
            setItems((prevItems) => {
                prevItems[index] = {...prevItems[index], [e.target.name]: e.target.value, price: item.price, shippingCharges: item.shippingCharges, cgst: item.cgst, sgst: item.sgst, code: item.code};
                return prevItems;
            });
        ...
        if(e.target.name === 'quantity'){
            setItems((prevItems) => {
                prevItems[index] = {...prevItems[index], [e.target.name]: (e.target.value)};
                return prevItems;
            });
        }
    ...
    const handleOnClick = (e) => {
        e.preventDefault();
        if (!customer.customerName || !customer.invoiceNo || !customer.sellerName || !customer.invoiceDate || !customer.orderDate || !customer.status ) {
        ...
        dispatch(addInvoice(customer)).then((res)=>{
            ...
            if(res.payload.errors){
                alert(res.payload.errors[0].msg)
            }
            else{
    ...
    return (
        <div className='container'>
            ...
            <div className="form-floating mx-2 " style={{ minWidth: "400px" }}>
                ...
                <div className='form-floating mx-2' style={{ minWidth: "400px" }}>
                    <input type="text" name="orderNo" value={customer.orderNo} onChange={handleOnChange} className='form-control' id='floatingInput' placeholder='Order No.' />
                ...
                <div className='form-floating mx-2' style={{ minWidth: "400px" }} >
                    <label htmlFor="billing">is Billing Address not same as Shipping Address</label>
